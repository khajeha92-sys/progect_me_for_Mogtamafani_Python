...
class WeaderClient:
    ...
    def __init__(self, latitude = 38.08, longitude = 46.29):
    ...
    def _weather_params(self):
        return{
            ...
            "current_weather" : True #'true'#'temperature_2m,wind_speed_10m,wind_direction_10m'#"true"
            }
...
class SafeAPIClient:
    ...
    def safe_api_call(self, url, params = None):
        try:
            ...
            print("Conectopn Error! Please check your internet.")
...
class CityWeatherApp:
    ...
    def display_weather(self, weather_data):
        if not weather_data:
            print('no weader data available.')
            return
...
def run_exercise_1():
    ...
    print('-' * 66)
    clint = WeaderClient()
    clint.compare_urllib_and_requests()

def run_exercise_2():
    print('\n Exercise 2 : safe API call with error handeling')
    print('-' * 66)
...
def run_city_weather_app():
    ...
    while True:
        city = input("Enter city name(or 'exit' to quite): ").strip()

        if city.lower() in ['exit', 'quite', 'q']:
            print('goodbye!')
            break

        if not city:
            print('Please entr a city name!')
            continue
